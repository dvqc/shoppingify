...
const ListInfo = ({ list }: { list: ListData }) => {
  ...
  return (
    <div className="flex flex-row items-center p-5 gap-8 bg-white rounded-xl shadow-base">
      ...
      >
        <span className="w-24">{dateFormatter.format(new Date(list.createdAt)).replaceAll("/", ".")}</span>
      </div>
      <div className={`px-2 w-24 text-center font-medium text-sm border-[1px]  rounded-lg ${conditionalStyle} `}>
        {list.status}
      </div>
      <a href="#" className="text-yellow1 font-extrabold text-xl">
        {">"}
      </a>
    </div>
  );
